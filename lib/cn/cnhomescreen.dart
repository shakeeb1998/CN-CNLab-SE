...
class CNHome extends StatefulWidget {
  ...
  String email;
  String photo;
  CNHome({@required this.uid,@required this.email,@required this.name,@required this.photo});
  ValueNotifier pos;
  @override
...
class _CNHomeState extends State<CNHome> {
  ...
  void initState() {
    ...
    var locationOptions = LocationOptions(accuracy: LocationAccuracy.bestForNavigation, distanceFilter: 0,timeInterval: 0);
  print('hello');
          ...
          if(position!=null)
            {
              ...
              Firestore.instance.collection(CnString.fusercollectiom).document(widget.uid).updateData({
...
//
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          children: <Widget>[
            ...
            InkWell(
                ...
                child: Padding(
                  ...
                  child: new ListTile(leading: Icon((Icons.exit_to_app)),title: new Text("Log Out")),
                )
            ),
            ...
            InkWell(
                ...
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: new ListTile(leading:  Icon((Icons.person_add)),title: new Text("Add Friends")),
                )
            ),
            new Divider(color: Colors.black,),
            ...
            InkWell(
                ...
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: new ListTile(leading: Icon((Icons.notification_important)),title: new Text("Requests")),
                )
            ),
            new Divider(color: Colors.black,),
            ...
            InkWell(
                onTap: () async {
                  ...
                  if(PermissionStatus.granted==permission)
                  {
                    Navigator.of(context).push(
                        CupertinoPageRoute(builder: (BuildContext context) => TextSEnder()));
                  }
                  else{
                ...
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: ListTile(leading: new Icon((Icons.message)),title: new Text("Invite Friends"),),
                )
            ),
            new Divider(color: Colors.black,),
      ...
      appBar: new AppBar(title:new Text(CnString.home),actions: <Widget>[

      ],
      ),
      body: new Center(
        child:StreamBuilder<QuerySnapshot>(
          stream:     Firestore.instance.collection(CnString.fusercollectiom).document(widget.uid).collection(CnString.ffrens).snapshots()
          ,
              ...
              default:
                return new ListView(
                  children: snapshot.data.documents.map((DocumentSnapshot document) {
                    return new CNHomeScreenCard(frenUid: document[CnString.uid],val: val,);
                  }).toList(),
                );
            }
          },
  ...
  acceptRequests(BuildContext context) async {
    ...
    Navigator.of(context).push(
        CupertinoPageRoute(builder: (BuildContext context) => CNAcceptRequests(uid: sp.getString(CnString.uid),)));
  }
